...
export default function QuizEngine({ questions: rawQ, title, color, onComplete, timePerQ = 60, enableTimer = true }) {
  const navigate = useNavigate();
  const questions = useMemo(() => shuffle(rawQ).slice(0, Math.min(rawQ.length, 30)), []);
  ...
  } = useQuiz(questions, enableTimer ? timePerQ : 99999);
    ...
    <div className={styles.wrap}>
      ...
      <div className={styles.bar}>
        ...
        <div className={styles.barRight}>
          ...
          {enableTimer && <TimerRing timeLeft={timeLeft} total={timePerQ}/>}
        </div>
      </div>
